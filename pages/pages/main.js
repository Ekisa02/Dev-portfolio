class PortfolioApp {
  constructor() {
    ...
    this.currentFilteredAchievements = []; // Track filtered achievements
  ...
  // NEW METHOD: Extract year from date string
  ...
  // NEW METHOD: Create year filter buttons
  createYearFilter() {
    const filterContainer = document.querySelector(".year-filter-container");
    if (!filterContainer) return;

    const years = [
      ...new Set(this.allAchievements.map((a) => this.extractYear(a.date))),
    ];
    years.sort((a, b) => b - a);

    const filterButtons = document.querySelector(".year-filter");
    ...
    allBtn.addEventListener("click", () => this.filterAchievements("all"));
    ...
    years.forEach((year) => {
      ...
      btn.addEventListener("click", () => this.filterAchievements(year));
      filterButtons.appendChild(btn);
    });
  }

  // NEW METHOD: Filter achievements by year
  filterAchievements(year) {
    ...
    let filtered;
    if (year === "all") {
      filtered = this.allAchievements;
    } else {
      filtered = this.allAchievements.filter(
        (a) => this.extractYear(a.date) === year
      );
    }

    this.currentFilteredAchievements = filtered;
  ...
  loadAchievements() {
    ...
    this.achievementsGrid.innerHTML = "";
    this.currentFilteredAchievements = this.allAchievements;
    ...
    // Update filter count
    const filterCount = document.getElementById("filterCount");
    if (filterCount) filterCount.textContent = this.allAchievements.length;
    ...
    this.loadMoreButton.addEventListener(
      "click",
      this.handleLoadClick.bind(this)
    );
  ...
  handleLoadClick() {
    const achievementsToLoad =
      this.currentFilteredAchievements || this.allAchievements;

    if (this.currentItemsShown >= achievementsToLoad.length) {
      this.achievementsGrid.innerHTML = "";
      this.currentItemsShown = 0;
      this.loadMoreItems();
    } else {
  ...
  loadMoreItems() {
    ...
    const newLimit = Math.min(
      this.currentItemsShown + this.itemsPerLoad,
      achievementsToLoad.length
    );

    const itemsToLoad = achievementsToLoad.slice(
      this.currentItemsShown,
      newLimit
    );
  ...
  updateButtonState() {
    ...
    if (achievementsToLoad.length <= this.itemsPerLoad) {
  ...
  createAchievementCard(achievement) {
    ...
    const year = this.extractYear(achievement.date);
    ...
    viewBtn.addEventListener(
      ...
      ((e) => {
        ...
        const safeTitle = achievement.title
          .replace(/[^a-z0-9]/gi, "_")
          .toLowerCase();
        const extension =
          achievement.certificateImage &&
          achievement.certificateImage.split(".").pop()
            ? achievement.certificateImage.split(".").pop()
            : "jpg";
  ...
  showCertificateModal(imgSrc, title, filename) {
    ...
    const isPdf = imgSrc ? imgSrc.toLowerCase().endsWith(".pdf") : false;
    ...
    if (isPdf) {
      ...
      modalBodyContent = `
            <img src="${imgSrc || "/placeholder-certificate.jpg"}" alt="${title} Certificate" class="certificate-image" />
        `;
    }
    ...
    // Download button
    modalOverlay
      .querySelector(".btn-modal-download")
      .addEventListener("click", () => {
        if (imgSrc) {
          ...
          alert("No certificate image available for download.");
        }
      });

    // Share button
    modalOverlay
      .querySelector(".btn-modal-share")
      .addEventListener("click", () => {
  ...
  setupEventListeners() {
    ...
    navLinks.forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        document
          .querySelector(`.mobile-nav-link[href="#${targetId}"]`)
